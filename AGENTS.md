...
*   **Configuration Base de Données (Strapi)** : L'application utilise SQLite situé dans `back/strapi_data/.tmp/data.db`. Ne définissez JAMAIS de valeurs de repli par défaut (ex: `DATABASE_PASSWORD`) dans `back/strapi/config/database.ts` : si l'env ne fournit pas les identifiants, l'application doit échouer silencieusement/sécuritairement.
...
*   **Mapping SQLite** : Dans Strapi 5, les noms d'attributs définis en camelCase dans `schema.json` (ex: `collectionName`) sont **automatiquement mapés** en snake_case dans les colonnes de la base de données (ex: `collection_name`).
