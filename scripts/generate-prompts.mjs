 ...
 * 
 * Usage:
 *   node scripts/generate-prompts.mjs --test             # Test sur la carte 001
 *   node scripts/generate-prompts.mjs --card 016         # Traite une carte spécifique
 *   node scripts/generate-prompts.mjs --range 001-010    # Traite une plage de cartes
 *   node scripts/generate-prompts.mjs --dry-run          # Affiche le body sans appeler le webhook
 *   node scripts/generate-prompts.mjs                    # Traite TOUTES les cartes
 *   node scripts/generate-prompts.mjs --no-backup        # Sans backup
 */
...
async function callWebhook(body) {
  ...
  try {
    ...
    try {
      ...
      // Si la réponse n'est pas du JSON valide, on essaie de la wrapper
      console.log('  ⚠️  Réponse non-JSON, tentative de parsing...');
      return { raw: text };
    }
  } finally {
 ...
 */
async function processCard(filePath, { dryRun = false, backup = true } = {}) {
  ...
  let newPrompt;
  if (result.raw) {
    console.log(`  ⚠️  Réponse brute stockée telle quelle`);
    newPrompt = result;
  } else if (result.prompt) {
    newPrompt = result.prompt;
  } else if (result.format && result.subject) {
    newPrompt = result;
  } else {
    newPrompt = result;
  }
