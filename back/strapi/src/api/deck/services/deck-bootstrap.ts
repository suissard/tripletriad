...
export async function bootstrapDecks(strapi: any) {
  try {
    ...
    if (fs.existsSync(decksDir)) {
      deckFiles = fs.readdirSync(decksDir).filter(f => f.endsWith('.json')).map(f => path.join(decksDir, f));
    }
    ...
    const allCards = await strapi.entityService.findMany('api::card.card', { fields: ['id'] });
    ...
    for (const filePath of deckFiles) {
      try {
        ...
        let deckCardIds = Array.isArray(deckData.cardIds) ? deckData.cardIds : [];
        ...
        const existingDecks = await strapi.entityService.findMany('api::deck.deck', {
          filters: { name: deckData.name },
        });

        if (existingDecks.length > 0) {
          const deck = existingDecks[0];
          console.log(`  🔄 Updating deck: "${deckData.name}" (ID: ${deck.id})`);
