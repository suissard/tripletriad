...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
  async openPack(ctx) {
    try {
      ...
      const commonCards = await strapi.entityService.findMany(
        "api::card.card",
        { filters: { rarity: "Common" } },
      );
      const rareCards = await strapi.entityService.findMany("api::card.card", {
        filters: { rarity: "Rare" },
      });
      const epicCards = await strapi.entityService.findMany("api::card.card", {
        filters: { rarity: "Epic" },
      });
      const legendaryCards = await strapi.entityService.findMany(
        "api::card.card",
        { filters: { rarity: "Legendary" } },
      );

      // Fallback: If no cards exist for a rarity, use all cards as a failsafe
      const allCardsEntities =
        await strapi.entityService.findMany("api::card.card");
      ...
      const allCards = {
        ...
        Legendary:
          legendaryCards.length > 0 ? legendaryCards : allCardsEntities,
      };
