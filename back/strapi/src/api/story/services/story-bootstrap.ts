...
export async function bootstrapStories(strapi: any) {
  try {
    ...
    console.log(
      `🚀 Starting two-pass story bootstrap for ${storyFiles.length} files...`,
    );
    ...
    for (const filePath of storyFiles) {
      try {
        ...
        let story;
        const existingStories = await strapi.entityService.findMany(
          "api::story.story",
          {
        ...
        if (existingStories.length > 0) {
          ...
          console.log(
            `  🔄 Updating story: "${storyData.title}" (ID: ${story.id})`,
          );

          // Delete old steps to ensure a clean re-seed
          const stepsToDelete = await strapi.entityService.findMany(
          ...
          story = await strapi.entityService.update(
            "api::story.story",
            story.id,
            {
              data: {
          ...
          console.log(
            `  🆕 Created story: "${storyData.title}" (ID: ${story.id})`,
          );
        }
        ...
        console.error(
          `  ❌ Failed to process file ${filePath} in Pass 1:`,
          err.message,
        );
      }
    }
      ...
      try {
        const createStep = async (stepData: any) => {
          const situations = (stepData.situations || []).map((s: any) => {
            ...
            if (
              cleanSit.__component === "story.situation-reward" &&
              cleanSit.rewardCards
            ) {
              cleanSit.rewardCards = (cleanSit.rewardCards as any[]).filter(
                (id) => cardIds.includes(Number(id)),
              );
            }
            // Validate decks in battle
            if (cleanSit.__component === "story.situation-battle") {
              if (cleanSit.enemyDeckName && deckMap.has(cleanSit.enemyDeckName))
                cleanSit.enemyDeck = deckMap.get(cleanSit.enemyDeckName);
              if (
                cleanSit.playerDeckName &&
                deckMap.has(cleanSit.playerDeckName)
              )
                cleanSit.playerDeck = deckMap.get(cleanSit.playerDeckName);
              delete cleanSit.enemyDeckName;
              delete cleanSit.playerDeckName;
              if (
                cleanSit.enemyDeck &&
                !deckIds.includes(Number(cleanSit.enemyDeck))
              )
                delete cleanSit.enemyDeck;
              if (
                cleanSit.playerDeck &&
                !deckIds.includes(Number(cleanSit.playerDeck))
              )
                delete cleanSit.playerDeck;
            }
            return cleanSit;
          });
        ...
        if (storyData.steps && Array.isArray(storyData.steps)) {
          console.log(
            `  📑 Seeding ${storyData.steps.length} steps for "${storyData.title}"...`,
          );
          for (const stepData of storyData.steps) {
            try {
              ...
              if (stepData.situations && Array.isArray(stepData.situations)) {
                ...
                console.log(
                  `    ✅ Created step: "${stepData.title}" (${stepSituations.length} situations)`,
                );
              } else {
                ...
                stepSituations = [];
                if (
                  stepData.startDialogue &&
                  stepData.startDialogue.length > 0
                ) {
                  stepSituations.push({
              ...
              console.error(
                `      ❌ Failed to create step "${stepData.title}":`,
                err.message,
              );
            }
          }
        } else if (
          storyData.situations &&
          Array.isArray(storyData.situations)
        ) {
          console.log(
            `  📑 Seeding multi-situation step for "${storyData.title}"...`,
          );
          try {
            const step = await createStep({
              title: storyData.title,
            ...
            console.log(
              `    ✅ Created single-step story (Step ID: ${step.id})`,
            );
          } catch (err: any) {
            console.error(
              `    ❌ Failed to create situations step for "${storyData.title}":`,
              err.message,
            );
          }
        }
        ...
        console.error(
          `  ❌ Failed to process story "${storyData.title}" in Pass 2:`,
          err.message,
        );
      }
    }
