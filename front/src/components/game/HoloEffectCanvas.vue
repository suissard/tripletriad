<template>
  <div ref="containerRef" class="holo-effect-canvas absolute inset-0 pointer-events-none rounded-[inherit] overflow-hidden z-[5]">
  </div>
</template>
...
const fragmentShaderStr = `
  ...
  void main() {
      ...
      for(int i = 0; i < MAX_LAYERS; i++) {
          ...
          float grain = random(vUv * (100.0 + uFoilScales[i] * 10.0) + uTime * 0.05);
          rainbow *= (0.7 + grain * 0.6); // Add texture
...
.holo-effect-canvas {
  width: 100%;
  height: 100%;
  z-index: 5; /* Matches holo-container */
}
:deep(canvas) {
  width: 100% !important;
